...
| `gate/scan-samples.json` | 스캐너 자체 검증 샘플 **27건** | 게이트 ② 회귀 |
...
### 1.4 파라미터 — D2·D5가 확정되면 기대값이 바인딩된다

**케이스 구조는 지금 완성됐지만, 기대 숫자는 두 값에 의존한다.**

| 파라미터 | 무엇 | 의존성 |
| --- | --- | :---: |
| `abs_threshold` | Net Benefit 절대 임계값(원) | **D2** |
| `rel_threshold` | Net Benefit 상대 임계값(비율) | **D2** |
| `scenario_delta` | 시나리오 증감 폭(비율) | **D5** |

확정 전에는 `"TBD"`로 남고, 확정 후 주입하면 전건이 바인딩된다.
...
**이 구조 덕분에 D2·D5를 기다리지 않고 게이트 배관을 먼저 만들 수 있다.** 임계값이 정해지면 명령 한 번으로 기대값이 채워진다.
...
| **PT-C** | AI 계산·판단 주체 표현 | **ADR-02** — 금액은 규칙 엔진이 산출한다 | block |
...
| 스캐너 구현 + 자체 검증 27건 | ✅ 완료 |
| **경계값 기대 숫자 바인딩** | ⬜ **D2·D5 확정 대기.** 명령 한 번으로 채워진다 |
