...
**배포 게이트 데이터는 `cardfit-gate-data-v1_0.md`(GTD-CARDFIT-001)에 있다** — 경계값 케이스 260건과 금지어 사전이며, D8을 해소한다. 스캐너는 구현·검증까지 완료됐다(`gate/`).

**테스트 케이스 27건은 `cardfit-testcase-v1_0.md`(STD-CARDFIT-001)에 있다** — 5장 추적성 매트릭스의 TC ID 본문이며, 배포 게이트(C-TEC-007a) 절차도 그 문서 4장에 있다.
...
**DEC-1과 DEC-3b는 D1·D9(규제 확인)와 함께 처리한다.** DEC-2는 확정됐다 — C-TEC-007a로 반영했고, 해당 게이트의 판정 절차는 테스트 명세서(`cardfit-testcase-v1_0.md`) 4장에 있다.
...
**RE-1~RE-8은 260건 경계값 케이스의 기대값을 확정하는 데도 필요하다.** 경계 *지점*은 정의됐으나(`cardfit-gate-data-v1_0.md`), 그 지점에서 나와야 할 *숫자*는 위 규칙이 정해진 뒤에 계산된다.
...
**테스트 케이스 ID의 본문은 `cardfit-testcase-v1_0.md`(STD-CARDFIT-001)에 있다** — 27건 전건에 사전조건·절차·기대 결과·판정 SLO가 정의돼 있고, 그중 6건이 P0(Guardrail 대응)이며 2건이 배포 게이트(C-TEC-007a)다.
...
| ~~D8~~ | 경계값 테스트 케이스 ≥ 200건 · 금지어 사전 | ✅ **해소 — 케이스 260건 · 금지어 6 카테고리**(`cardfit-gate-data-v1_0.md`) | 기대 숫자 바인딩만 D2·D5 대기 |
