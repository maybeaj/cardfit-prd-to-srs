...
| **C-TEC-007** | 배포 및 인프라 관리는 **Vercel 플랫폼**으로 단일화하며, CI/CD 설정 없이 **Git Push만으로 배포를 자동화**한다 | REQ-NF-001 · REQ-NF-003 |
...
**갈래 ② C-TEC-007(CI/CD 없음)과 정면 충돌 (2건)**

C-TEC-007은 *"CI/CD 설정 없이 Git Push만으로 배포"* 를 규정한다. 그런데 **배포 전 검증을 요구하는 항목이 2건 있다.** CI가 없으면 검증을 건너뛴 코드가 그대로 배포된다.

| 요구사항 | 배포 전에 통과해야 하는 것 | 대응 |
| :---: | --- | --- |
| **REQ-NF-002** | 경계값 회귀 **200건**을 **배포마다** 실행. 계산 오류율 ≤ 0.1%는 **GR1 Guardrail**이다 | 최소한의 게이트가 필요하다. **C-TEC-007을 "파이프라인 구성 없이"로 좁게 읽고, GitHub Actions 등 저비용 검사 1개는 허용**하도록 제약을 완화하는 편을 권한다 |
| **REQ-FUNC-009** | 금지어 **배포 전 정적 스캔**. 적발 0건은 **GR4 Guardrail**이다 | 상동. 특히 C-TEC-006이 **환경 변수만으로 모델 교체**를 허용하므로, 스캔 없이 배포되는 경로가 실제로 열려 있다 |

> **Guardrail 5건 중 2건이 배포 게이트를 전제로 설계됐다.** C-TEC-007을 문자 그대로 지키면 이 둘은 "사후 발견"만 가능해지고, *"1건이라도 발생 시 즉시 중단"* 이라는 판정 기준이 사전 차단에서 사후 중단으로 성격이 바뀐다. **제약과 요구사항 중 무엇을 고칠지 결정이 필요하다.**
...
| **DEC-2** | 배포 전 검증 게이트를 허용하는가 | ⓐ C-TEC-007 완화 — 검사 1개 허용 · ⓑ 제약 유지 — **GR1·GR4를 사후 중단으로 성격 변경** | 배포 게이트가 없어 Guardrail 2건이 사전 차단 기능을 잃는다 |
| **DEC-3** | 사용자 금융정보를 외부 AI에 전송하는가 | ⓐ 익명화 후 전송 · ⓑ 동의 범위에 국외 이전 명시 · ⓒ AI 설명 기능 제외 | REQ-FUNC-006의 AI 설명 부분을 확정할 수 없다 |

**DEC-1과 DEC-3은 D1·D9(규제 확인)와 함께 처리한다.** DEC-2는 팀 내부 결정으로 즉시 가능하다.
...
| **D12** | **DEC-2** 배포 전 검증 게이트 허용 여부 (C-TEC-007 완화) | 🔴 미정 | REQ-NF-002 · REQ-FUNC-009 · **GR1·GR4의 사전 차단 성격** |
...
**D11~D15는 1.5.2 스택 적합성 검토에서 도출된 것이다.** D11(마이데이터 연동 가능성)은 D1(인가 vs 제휴)과 같은 사안의 기술 측면이라 **함께 결정**해야 하고, D12는 팀 내부 결정이라 **즉시 처리 가능**하다.
