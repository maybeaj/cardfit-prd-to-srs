...
**근거 문서:** TASK-CARDFIT-001 v3.0 (`[태스크 리스트] CardFit.md`)
...
`[태스크 리스트] CardFit.md`는 태스크 **58건을 한 줄씩** 나열한 목록이다. 이 문서는 그 한 줄을 **GitHub 이슈 한 장의 상세 명세**로 펼치는 템플릿과 작성 규칙, 그리고 **어떤 순서로 펼칠지**를 정의한다.
...
**`### 2.4` References 첫 줄에 태스크 리스트를 넣는다** — `/docs/[태스크 리스트] CardFit.md#ct-01`. 이슈에서 목록으로 되짚어 갈 경로가 있어야 한다.
...
| 항목 | 실제 경로 | 언제 넣나 |
| --- | --- | --- |
| SRS 문서 | `/docs/[SRS 문서] CardFit (한글).md` + 절 번호 | **전 태스크** |
| 시퀀스 다이어그램 | `/docs/[설계 문서] CardFit (한글).md` §5 | BE·FE |
| 데이터 모델 (ERD) | `/docs/[SRS 문서] CardFit (한글).md` §6.4.1 | DA·BE |
| API 명세 | `/docs/[SRS 문서] CardFit (한글).md` §6.1.1~6.1.3 | CT·MK·BE·FE |
| 테스트 케이스 | `/docs/[테스트 명세서] CardFit (한글).md#tc-func-004` | AC가 있는 전 태스크 |
| **계산 명세** | `/docs/[계산 명세서] CardFit (한글).md` + 절 번호 | **계산·게이팅·배분·근거** |
| **픽스처 명세** | `/docs/[픽스처 데이터 명세] CardFit (한글).md` + 절 번호 | **`mode:fixture` 태스크** |
...
*근거 문서: `[태스크 리스트] CardFit.md` (TASK-CARDFIT-001 v3.0)*
