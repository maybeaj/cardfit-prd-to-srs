...
**참조 문서:** SDD-CARDFIT-001 (`[SDD]cardfit-design.md`) · GTD-CARDFIT-001 (`[GTD]cardfit-gate-data.md`)
...
SRS 본문에 넣지 않은 이유는 두 가지다. 표준 양식(`[SRS] AD-Core-Platform`)이 추적성 매트릭스에 **TC ID만 나열**하고 본문을 두지 않으며, 테스트 케이스는 요구사항이 아니라 **검증 산출물**이다. SRS 11장은 검증 *접근*(29148 9.6.19)을 규정하고, 이 문서는 그 접근의 *실행 단위*를 규정한다.
...
| ~~D8~~ 경계값 케이스 · 금지어 사전 | ✅ **해소** — `[GTD]cardfit-gate-data.md` (케이스 260건 · 사전 6 카테고리). 기대 숫자는 D2·D5 확정 시 바인딩 |
...
*입력 문서: `[SRS]cardfit-srs-v1_0.md` · `[SDD]cardfit-design.md`*
