...
**참조 문서:** `[SRS 문서] AD-Core-Platform (한글).md` (양식 기준)
...
**배포 게이트 데이터는 `[배포 게이트 데이터] CardFit (한글).md`(GTD-CARDFIT-001)에 있다** — 경계값 케이스 260건과 금지어 사전이며, D8을 해소한다. 스캐너는 구현·검증까지 완료됐다(`tools/`).

**테스트 케이스 27건은 `[테스트 명세서] CardFit (한글).md`(STD-CARDFIT-001)에 있다** — 5장 추적성 매트릭스의 TC ID 본문이며, 배포 게이트(C-TEC-007a) 절차도 그 문서 4장에 있다.

**전체 설계 산출물은 `[설계 문서] CardFit (한글).md`(SDD-CARDFIT-001)에 있다** — 유스케이스 명세, 컴포넌트 책임 정의, 클래스 다이어그램 3종, 시퀀스 다이어그램 10종, 순서도 5종, 상태 기계 4종, SRS↔설계 추적 매트릭스. 본 문서의 다이어그램은 그중 요구사항 이해에 필요한 것만 발췌한 것이다.
...
| # | 결정 사항 | 선택지 | 결정하지 않으면 |
| :---: | --- | --- | --- |
| **DEC-1** | 마이데이터 연동을 Vercel에서 할 수 있는가 | ⓐ 가능 — 현행 유지 · ⓑ 불가 — **연동 전용 서버 분리**(C-TEC-001·007 예외) | REQ-FUNC-002 착수 불가 = 계산 불성립 |
...
| **DEC-3b**<br>근거 설명 | **사용자의 카드·금액 정보**를 외부 AI에 전송하는가 | ⓐ 식별정보 제거 후 금액·규칙만 전송 · ⓑ 동의 범위에 국외 이전 명시 · ⓒ 이 기능만 제외 | REQ-FUNC-006의 근거 설명 부분을 확정할 수 없다 |
...
**DEC-1과 DEC-3b는 D1·D9(규제 확인)와 함께 처리한다.** DEC-2는 확정됐다 — C-TEC-007a로 반영했고, 해당 게이트의 판정 절차는 테스트 명세서(`[테스트 명세서] CardFit (한글).md`) 4장에 있다.
...
> **그림 읽는 법** — 상자는 기능 단위 코드 덩어리(컴포넌트), 큰 테두리는 계층이다. 화살표는 "누가 누구를 부르나"를 뜻한다. **노란 테두리 안에는 AI가 들어갈 수 없다**(9장 ADR-02). 상세 컴포넌트 설계는 `[설계 문서] CardFit (한글).md` 2장에 있다.
...
> **본 명세는 규칙 엔진이 "무엇을 입력받아 무엇을 내놓는가"까지만 정의했다.** 그 안에서 **어떤 순서로 어떤 식을 적용하는가**는 정의되지 않았고, 이는 기술 결정이 아니라 **기획·상품 결정**이다. 아래가 비어 있으면 REQ-FUNC-003·004는 구현자 재량으로 채워지며, 그 재량이 계산 결과를 바꾼다.
...
**RE-1~RE-8은 260건 경계값 케이스의 기대값을 확정하는 데도 필요하다.** 경계 *지점*은 정의됐으나(`[배포 게이트 데이터] CardFit (한글).md`), 그 지점에서 나와야 할 *숫자*는 위 규칙이 정해진 뒤에 계산된다.
...
> **그림 읽는 법** — 상자 하나가 **클래스**(코드에서 만들 부품 하나)다. 상자 안 윗칸은 그 부품이 가진 데이터, 아랫칸은 그 부품이 하는 일이다. 화살표는 "이 부품이 저 부품을 쓴다"는 뜻이다. **아래 표의 `구현 클래스` 열이 이 그림에서 나왔다.** 전체 클래스 설계는 `[설계 문서] CardFit (한글).md` 3장에 있다.
...
**테스트 케이스 ID의 본문은 `[테스트 명세서] CardFit (한글).md`(STD-CARDFIT-001)에 있다** — 27건 전건에 사전조건·절차·기대 결과·판정 SLO가 정의돼 있고, 그중 6건이 P0(Guardrail 대응)이며 2건이 배포 게이트(C-TEC-007a)다.

모듈 열은 3장이 정의한 내부 구성요소, 구현 클래스 열은 `[설계 문서] CardFit (한글).md` 3장 클래스 다이어그램에서 가져온 것이다. 각 요구사항에 대응하는 인수 기준과 검증 실험은 4.1·4.2·4.3의 검증 방식 및 인수 기준 열에 기재한다.
...
> **그림 읽는 법** — 사람 모양이 **행위자**(서비스를 쓰는 사람), 둥근 상자가 그 사람이 **하는 일**이다. 점선은 외부 시스템 연결이다. 유스케이스 상세 명세는 `[설계 문서] CardFit (한글).md` 1장에 있다.
...
| ~~D8~~ | 경계값 테스트 케이스 ≥ 200건 · 금지어 사전 | ✅ **해소 — 케이스 260건 · 금지어 6 카테고리**(`[배포 게이트 데이터] CardFit (한글).md`) | 기대 숫자 바인딩만 D2·D5 대기 |
| D9 | 모집인 등록·재무자문 규제 분류 | 미확인 | REQ-NF-004 · REQ-NF-006 보존기간 · REQ-FUNC-009 |
| D10 | 실제 사용자 인터뷰 (모의 인터뷰 대체) | 미실시 | A4 확정 · 8.2 스토리 우선순위 |
| **D11** | **DEC-1** 마이데이터 연동을 Vercel에서 수행 가능한가 (고정 IP·인증서·물적설비) | 🔴 미확인 | **REQ-FUNC-002 전체** · C-TEC-001·007 예외 여부 |
| ~~D12~~ | **DEC-2** 배포 전 검증 게이트 허용 여부 | ✅ **확정 — 허용**(C-TEC-007a) | 해소됨 |
| **D13** | **DEC-3** 사용자 금융정보의 외부 AI 전송 방식 | 🔴 미정 | REQ-FUNC-006 AI 설명 부분 |
| D14 | 사용자 발송 채널 · 실시간 알림 · 이벤트 분석 · APM 도구 선정 | 미정 | REQ-FUNC-010 · REQ-NF-001·003·009 · Guardrail 5건 |
| D15 | 서버리스 실행 시간 상한 내 3개 시나리오 계산 가능성 실측 | 미실시 | REQ-FUNC-003·004 · ADR-03 |
| **D16** | **규칙 엔진 계산 명세 RE-1~RE-8** (적용 순서 · 제외 대상 · 산정 기간 · 전월 정의 · **전환비용 산정** · 조합 생성 규칙 · 후보 풀 · 중복 혜택) | 🔴 **미정 — 기획 결정** | **REQ-FUNC-003·004·005 전체** · 경계값 260건의 기대값 · **D2 선행** |

**D1·D2·D4·D5가 미정인 동안 REQ-FUNC-002·003·004·006은 착수할 수 없다** — Must 7건 중 4건이 선결 항목에 막혀 있다는 뜻이고, 이것이 본 명세의 실질적 착수 조건이다.

**D16은 이 명세의 가장 큰 공백이다.** D2(임계값)는 숫자 하나지만, D16은 **계산 로직 자체**다. 순서가 다르면 같은 입력에 다른 금액이 나오므로, REQ-NF-002(재계산 불일치 0건)를 만족해도 **틀린 값으로 일관되게 틀릴 수 있다.** 상세는 4.1.0에 있다.

**D11~D15는 1.5.2 스택 적합성 검토에서 도출된 것이다.** D11(마이데이터 연동 가능성)은 D1(인가 vs 제휴)과 같은 사안의 기술 측면이라 **함께 결정**해야 한다. **D12는 확정됐다** — 배포 전 검증 게이트를 허용하며, C-TEC-007a로 명세했다.
