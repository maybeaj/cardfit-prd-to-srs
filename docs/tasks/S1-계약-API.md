# [태스크 명세 · 배치 1] 계약 — API (CT-01 · CT-02)
...
**`CT-01` 하나가 5개 그룹을 막고 있다**(`CT-02` · `MK-01` · `MK-02` · `BE-11` · `BE-19`). 계약이 없으면 BE는 응답 형태를 임의로 정하고, FE는 그것을 기다리며, Mock은 만들 대상이 없다.
...
- 데이터 모델 (enum): [`§6.2 데이터 모델 정의`](#62-데이터-모델-정의) — `ScenarioType` · `CalculationStatus` · `GatingResult` · `TransitionCostItem` · `OutcomeLogStatus`
- 시퀀스 다이어그램: [`/docs/[설계 문서] CardFit (한글).md` §5.1 SD-01 정상 계산](#51-sd-01-정상-계산--3개-시나리오-사전-계산)
- 상태 전이: [`§6.5 상태 전이 규칙`](#65-상태-전이-규칙)
...
Scenario 8: AI 설명 실패해도 응답이 유효
- Given: `termsSummary: null` · `rationale: null` 이고 `items` 6건이 담긴 페이로드가 주어짐
- When: `EvidenceResponse` 스키마로 파싱함
- Then: 검증을 통과한다. **AI는 보조 기능이므로 실패해도 근거는 노출된다.** (ADR-02)

Scenario 9: 미반영 항목 키 생략 거부
- Given: `unreflectedItems` 키가 아예 없는 페이로드가 주어짐
- When: `EvidenceResponse` 스키마로 파싱함
- Then: 검증에 실패한다. 빈 배열과 키 부재를 구분해 누락률 0% 표기를 강제한다.

Scenario 10: 완주 계측 응답에 실행 개입 필드 부재
- Given: `CompletionResponse` 스키마의 필드 목록이 주어짐
- When: 필드명을 검사함
- Then: `nextAction` · `retryUrl` · `reminderAt` 등 재시도·독려를 유도하는 필드가 **0개**다. (ADR-04 · GR4)
...
- 보안(완주): `incompleteReason`은 사용자 자유 입력이라 마스킹 정책 대상임을 주석에 명시한다(REQ-NF-004).

## 🏁 Definition of Done (DoD)
- [ ] 모든 Acceptance Criteria(10건)를 충족하는가?
- [ ] 세 도메인의 계약 스냅샷 테스트가 추가되었고 통과하는가?
- [ ] 배포 게이트 ①·②를 통과하는가? (C-TEC-007a)
- [ ] 의존성 경계 린트를 통과하는가? (ADR-02)
- [ ] Linter 경고가 없는가?
- [ ] SRS §6.1.3의 필드·규칙과 계약이 1:1로 대응하는가? (누락·추가 필드 0건)
- [ ] 완주 응답 스키마에 실행 개입을 유도하는 필드가 0개인가? (GR4)
- [ ] `python3 tools/verify_docs.py` 통과

## 🚧 Dependencies & Blockers
- Depends on: None
- Blocks: CT-02 · MK-01 · MK-02 · BE-11 · BE-19
```
...
name: Feature Task
about: SRS 기반의 구체적인 개발 태스크 명세
title: "[Feature] CT-02: 공통 에러 코드 체계 구현 (CF-4001 ~ CF-2001)"
...
## 🚧 Dependencies & Blockers
- Depends on: CT-01 (공통 응답 봉투 타입)
- Blocks: MK-02 · BE-12 · BE-19
```
...
| 즉시 착수 가능 | 근거 |
| :---: | --- |
| **MK-01 · MK-02** | 계약이 확정되면 Mock을 만들 대상이 생긴다 |
| BE-11 · BE-19 | CT-01 완료 시 |
| BE-12 | CT-02 완료 시 |

**Mock 2건이 열리는 것이 이 배치의 실질적 효과다.** Mock이 나오면 FE 7개 그룹의 백엔드 직렬 의존이 풀린다.
